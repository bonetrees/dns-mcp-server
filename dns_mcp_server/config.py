...
class DNSServerConfig:
    ...
    dns_query_all_concurrency: int = (
        3  # concurrent queries to same resolver in query_all
    )
...
def _validate_config():
    ...
    assert (
        0 < config.high_failure_rate_threshold < 1
    ), "Failure rate threshold must be between 0 and 1"

    # Validate resolver configurations
    for resolver_name, nameservers in RESOLVER_CONFIGS.items():
        assert isinstance(
            nameservers, list
        ), f"Resolver {resolver_name} must have list of nameservers"
        assert (
            len(nameservers) > 0
        ), f"Resolver {resolver_name} must have at least one nameserver"
